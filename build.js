...
var insults = [" You swine. You vulgar little maggot. You worthless bag of filth. As we say in Texas, you couldnt pour water out of a boot with instructions printed on the heel. You are a canker, an open wound. I would rather kiss a lawyer than be seen with you. You took your last vacation inthe Isles of Langerhan.", " You're a putrescent mass, a walking vomit. You are a spineless little worm deserving nothing but the profoundest contempt. You are a jerk, a cad, a weasel. Your life is a monument to stupidity. You are a stench, a revulsion, a big suck on a sour lemon.", " You are a bleating foal, a curdled staggering mutant dwarf smeared richly with the effluvia and offal accompanying your alleged birth into a hostile world. You are an insensate, blinking calf, meaningful to nobody, abandoned by the puke-drooling, giggling beasts who sired you and then died of shame in recognition of what they had done. They were a bit late.",
    ...
    " On an intelligence scale of 1 to 10 (10 corresponding to the highest attainable IQ) you're rating is so far into negative numbers that one would need to travel into another quantum reality in order to even catch who keeps slamming into the picture window trying to get that other bird it keeps seeing", "You are so clueless that if you dressed in a clue skin, doused yourself in clue musk, and did the clue dance in the middle of a field of horny clues at the height of clue mating season, you still would not have a clue. If you were a movie you would be a double feature; _Battlefield_Earth_ and _Moron_Movies_II_. You would be out of focus.",
    " To call you a parasite would be injurious to the thousands of honest parasitic species. You are worse than vermin, for vermin does not pretend to be what it is not. You are truly human garbage. ", "You are a fiend and a sniveling coward, and you have bad breath. You are the unholy spawn of a bandy-legged hobo and a syphilitic camel. You wear strangely mismatched clothing with oddly placed stains. You are degenerate, noxious and depraved. I feel debased just knowing that you exist. I despise everything about you, and I wish you would go away. You are jetsam who dreams of becoming flotsam. You won't make it. I beg for sweet death to come and remove me from a world which became unbearable when the bioterrorists designed you.",
    " You smarmy lagerlout git. You bloody woofter sod. Bugger off, pillock. You grotty wanking oik artless base-court apple-john. You clouted boggish foot-licking half-twit. You dankish clack-dish plonker. You gormless crook-pated tosser. You bloody churlish boil-brained clotpole ponce. You craven dewberry pisshead cockup pratting naff. You cockered bum-bailey poofter. You gob-kissing gleeking flap-mouthed coxcomb. You dread-bolted fobbing beef-witted clapper-clawed flirt-gill. You jetere steatopygous pilgarlick hircine whigmaleerious rhadamanthine lintlicker. I refer you to the reply given in the case of Arkell v. Pressdram.", " you are a flatulent, whelk obsessed bottom gurgler with the style and grace of a leprous baboon. if I had only known that you were possessed of such a singular body aroma, I should have attempted to conduct our meeting through the medium of signal fires, you barely sane, badger-throwing,egg-headed malodourus midget of questionable parentage.",
        ...
        beforeChannelMessage: function(message, channel, html) {
            ...
            if (banned_users.indexOf(client.id(vUserSentName)) !== -1) {
                ...
                if (vCommand == "google") {
                    ...
                    var search = vData[0]
                    var result = sys.synchronousWebCall("https://www.google.com/search?q=" + search + "&ie=utf-8&oe=utf-8").toString().split("-", 2)
                    print(result)
                }
                if (vCommand == "namebot") {
                    if (controllers.indexOf(vUserSentName.toLowerCase()) !== -1) {
                ...
                if (vCommand == "makecontroller") {
                    ...
                    if (vUserSentName.toLowerCase() == "heark") {
                        var vData = vCommandData.split(":", 2);
                        var name = vData[0];
                        controllers.push(name.toLowerCase())
                        client.network().sendChanMessage(channel, vUserSentName + " added " + name + " to the bot controller list!");
                    } else {
                        client.network().sendChanMessage(channel, vgBotName + "You don't have permission to use this command");
                    }
                }
             ...
             if (vCommand == "translate") {
                        ...
                        if(tLang == undefined){
                         ...
                         tString[x] = translated.tuc[x].text;
                            ...
                            var tStringToPrint = tString[0];
                           if(tStringToPrint == undefined ){
                           	client.network().sendChanMessage(channel, "Failed to load data for "+tWord)
                           } else {
                           	client.network().sendChanMessage(channel, tStringToPrint);
                           }
                ...
                if (botOnline == true) {
                   ...
                   client.network().sendChanMessage(channel, "Plot: "+ vDefMessageInfo);
                            ...
                            if (vCommand == "shoot") {
                                ...
                                client.network().sendChanMessage(channel, "/me ** " + vUserSentName + " shot " + vTarget + " with  " + gun[sys.rand(0, gun.length)])
                            }
                            if (vCommand == "match") {
                    ...
                    if (vCommand == "fight") {
                        ...
                        var kill = [winner+" commited a felony on " + loser + "", winner+" read a book to " + loser + "", winner+" drove their car into " + loser + "'s face.", winner+"shot" + loser + "", winner+" gave an overdose of viagra to " + loser + "", "" + loser + " was forced to have anal intercourse with a stallion by " + winner + ".", winner+" forced a toothpick down" + loser + "'s throat"];
